...
let inputMonitoring = { if #available(macOS 10.15, *) { return CGPreflightListenEventAccess() }; return true }()
guard let tap=CGEvent.tapCreate(tap:.cgSessionEventTap,place:.headInsertEventTap,options:.listenOnly,eventsOfInterest:mask,callback:callback,userInfo:nil)else{emit("permission",["granted":false,"accessibility":AXIsProcessTrusted(),"input_monitoring":inputMonitoring]);emit("gap",["reason":"event_tap_create_failed"]);exit(78)}
emit("permission",["granted":true,"accessibility":AXIsProcessTrusted(),"input_monitoring":inputMonitoring])
