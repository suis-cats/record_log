...
guard let tap=CGEvent.tapCreate(tap:.cgSessionEventTap,place:.headInsertEventTap,options:.listenOnly,eventsOfInterest:mask,callback:callback,userInfo:nil)else{emit("permission",["granted":false]);emit("gap",["reason":"event_tap_create_failed"]);exit(78)}
