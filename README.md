...
Accessibilityが未許可の場合は「計測前テスト」→「Accessibilityを許可」を押す。macOSの設定画面が開き、許可対象として認識されている実行ファイルのパスが画面に表示される。表示されたResearch Recorderをアクセシビリティ一覧でオンにし、アプリを完全終了して再起動する。別の場所にある旧ビルドへの許可では取得できない。
